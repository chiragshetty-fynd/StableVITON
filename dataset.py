...
class VITONHDDataset(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        assert not (
            self.data_type == "train" and self.pair_key == "unpaired"
        ), f"train must use paired dataset"
    ...
    def __getitem__(self, idx):
        ...
        agn = imread(
            opj(self.drd, self.data_type, "agnostic", self.im_names[idx]),
            self.img_H,
            self.img_W,
        )
        agn_mask = imread(
            opj(
                ...
                self.data_type,
                "agnostic-mask",
                self.im_names[idx].replace(".jpg", "_mask.png"),
            ),
            self.img_H,
            self.img_W,
            is_mask=True,
