...
class DDIMSampler(object):
    ...
    def sample(
        ...
        **kwargs,
        ...
        samples, intermediates, cond_output_dict = self.ddim_sampling(
            conditioning,
            size,
            callback=callback,
        ...
        if cond_output_dict is not None:
            ...
            if self.model.use_noisy_cond:
                ...
                sqrt_one_minus_at = torch.full(
                    (b, 1, 1, 1), sqrt_one_minus_alphas[0], device=device
                )

                c = cond_output_dict["cond_input"]
                e_t = cond_output
                pred_c0 = (c - sqrt_one_minus_at * e_t) / a_t.sqrt()
                dir_ct = (1.0 - a_prev - sigma_t**2).sqrt() * e_t
    ...
    def p_sample_ddim(
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.0:
            ...
            model_uncond, cond_output_dict_uncond = self.model.apply_model(
            ...
            if cond_output_dict_cond is not None:
                ...
                for k in cond_output_dict_cond.keys():
                    cond_output_dict[k] = torch.cat(
                        [cond_output_dict_uncond[k], cond_output_dict_cond[k]]
                    )
            else:
    ...
    def encode(
        ...
        for i in tqdm(range(num_steps), desc="Encoding Image"):
            ...
            if unconditional_guidance_scale == 1.0:
                ...
                noise_pred = (
                    e_t_uncond
                    + unconditional_guidance_scale * (noise_pred - e_t_uncond)[0]
                )
